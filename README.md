...
- Expo SecureStoreへのaccess token / refresh token保存
...
モバイル版のGoogle / GitHubログイン、EAS Build、ストア配布は未実装です。
...
- bcryptでパスワードを検証
- 成功時に短期access tokenと長期refresh tokenを発行
- refresh tokenは平文保存せず、`ApiSession.refreshTokenHash` にSHA-256ハッシュとして保存
- 以降のAPIで `Authorization: Bearer <accessToken>` を付与
- access token / refresh tokenはExpo SecureStoreへ保存
- access tokenの有効期限は15分、refresh tokenの有効期限は30日
- access token期限切れ時は `/api/mobile/auth/refresh` でrefresh token rotationを行い、新しいaccess token / refresh tokenを保存
- ログアウト時は `/api/mobile/auth/logout` で対象のApiSessionを `revokedAt` により無効化

`MOBILE_AUTH_SECRET` が未設定の場合、モバイル用access tokenの署名にも `AUTH_SECRET` を使います。refresh tokenはログ出力やクライアント外部への送信を避け、SecureStoreにのみ保存してください。
...
| `POST` | `/api/mobile/auth/refresh` | refresh tokenを検証し、新しいaccess token / refresh tokenを発行 |
