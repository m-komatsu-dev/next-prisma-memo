...
| 配布 | Vercel | Expo Go / EAS Buildを想定 |
...
実機でExpo Goから接続する場合、`localhost` ではなくPCのLAN IPを指定してください。詳しくは [mobile/README.md](mobile/README.md) と [ENVIRONMENT.md](ENVIRONMENT.md) を参照してください。
...
- Cron認証をquery secretからheader認証へ移行
- 本番ドメイン確定後のCSP段階導入
- EAS Buildによる内部配布フロー整備
